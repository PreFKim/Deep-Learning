...
def shufflenet_unit(input,stride,ratio=0.5):
  if stride == 1:
    c = int(input.shape[-1] * ratio)
    x = input[:,:,:,0:c]
    shortcut = input[:,:,:,c:input.shape[-1]]
  else :
    c = input.shape[-1]
    x = input
    ...
    shortcut = keras.layers.Conv2D(input.shape[-1],1,1,'same')(shortcut)
    shortcut = keras.layers.BatchNormalization()(shortcut)
...
def shufflenet_v2(s=1):
  ...
  for i in range(len(repeat)):
    x = shufflenet_unit(x,2)
    for _ in range(repeat[i]):
      x = shufflenet_unit(x,1)
    channels *= 2
